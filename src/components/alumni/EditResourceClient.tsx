...
import { updateResourceAction } from "@/app/(alumni)/alumni/actions";
...
export default function EditResourceClient({
  userId,
  isAdmin,
  resource,
}: EditResourceClientProps) {
  ...
  const [existingFileUrl, setExistingFileUrl] = useState<string | null>(resource.file_url || null);
  const [existingFileType, setExistingFileType] = useState<string | null>(resource.file_type || null);
  const [existingFileSize, setExistingFileSize] = useState<number | null>(resource.file_size || null);
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!file && !existingFileUrl) {
      showAlert(
        "error",
        "File Kosong!",
        "Harap pilih file yang ingin diupload."
      );
      return;
    }
    ...
    try {
      ...
      showAlert(
        "success",
        "Berhasil!",
        "Resource kamu telah berhasil diperbarui."
      );
    } catch (error: any) {
      console.error(error);
      showAlert(
        "error",
        "Oops!",
        error?.message || "Gagal memperbarui resource. Silakan coba lagi."
      );
    } finally {
      setLoading(false);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <p className="text-sm text-white/85">Upload File* (Atau biarkan untuk menggunakan file saat ini)</p>
            ...
            >
              ...
              {file ? (
                ...
                <div className="flex flex-col items-center p-6 text-center">
                  ...
                  <p className="mt-1 text-sm text-[#7dd3d3]">
                    Format: {existingFileType?.toUpperCase()} • {existingFileSize ? `${(existingFileSize / (1024 * 1024)).toFixed(2)} MB` : ""}
                  </p>
